...
Sometimes, it is good practice to give an explicit name to unused arguments, for clarity. However, you should still adhere to the previous rule and prepend the name with an underscore, which is still an explicit signal that you know the var isn't supposed to be used. This is particularly true when destructuring complex maps.
...
(defn myfun2 [a {:keys [b c] :as _mymap}]
 (println a b c))
----
